...
const postsSlice = createSlice({
	...
	reducers: {
		...
		postUpdated(state, action) {
			const groupArray = state.data[action.payload.targetEl.group]
			const newArray = groupArray.map(item => {
				if (item.id === action.payload.post.id) return { ...item, description: action.payload.newDescription }
				return item
			})
			state.data[action.payload.targetEl.group] = newArray
		},
...
export function updateElementPost(post, newDescription) {
	return async (dispatch, getState) => {
		...
		try {
			...
			const arrayKeys = Object.keys(getState().posts.data)
			if (arrayKeys.length) {
				const arrayPosts = []
				arrayKeys.forEach(key => {
					arrayPosts.push(...getState().posts.data[key])
				})
				const targetEl = arrayPosts.find(el => el.id === post.id)
				if (targetEl) {
					dispatch(postUpdated({ post, newDescription, targetEl }))
				}
			}
		} catch (err) {
			const { message } = err
